...
export class AuthService {
  ...
  constructor(private _apiHandler: ApiHandlerService, private _userService: UserService) {
  }
  ...
  logout(): Observable<string> {
    // API for logout to destroy token
    return this._apiHandler.callService(`customer/${this._userService.get().id}`, RequestMethod.Get)
  ...
  authenticate(email: string, password: string): Observable<string> {
    return this._apiHandler.callService("login/user", RequestMethod.Post, {email: email, password: password})
      ...
      .do((res: string) => {
        ...
        localStorage.setItem('userId', data.account_details.id);
